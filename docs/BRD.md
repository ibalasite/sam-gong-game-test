...
| **文件版本** | v0.6-draft |
| **狀態** | DRAFT（BRD Fixer Round 5 完成）|
| **作者** | Evans Tseng（由 /devsop-idea 自動生成） |
| **日期** | 2026-04-22 |
| **建立方式** | /devsop-idea 自動生成；v0.6-draft 由 /devsop-autodev STEP-01 Review Round 5 自動修正 |
...
> **核心目標宣言（F1）：** 在 **4 個月內**上線一款 Server-authoritative 即時多人三公遊戲，以「絕對公平」為差異化核心，於 Launch+6M 達成 Peak CCU ≥ 500、DAU ≥ 2,000、於 Launch+3M 達成公平性 NPS ≥ 70，並於 Launch+9M 建立合規虛擬籌碼變現模式，付費率 ≥ 3%，奠定 Sayyo Games 華人休閒遊戲平台基礎。
...
| O2 | 留存 | 新手引導完成率 ≥ 60%（§7） | REQ-012 Tutorial, REQ-010, REQ-011 | BDD Scenario S-012 | 🔲 待填 |
...
| 新手引導教學（Tutorial Onboarding）| Must Have | O2 | AC：首次登入玩家的教學完成率 ≥ 60%（Launch+1M量測，分母為當月首次登入帳號數）；教學包含：三公規則說明、第一局模擬牌局（不消耗籌碼）、籌碼系統說明；完成後解鎖正式對戰入口 |
...
6. 籌碼分配（含 5% 抽水），Server 廣播最終狀態至所有 Client
...
| **同點比大小（F3 確定規則）** | 同點數時：**第一順位比最大單張花色**（黑桃 > 紅心 > 方塊 > 梅花）；花色相同時，再比最大單張點數（K>Q>J>10>9>8>7>6>5>4>3>2>A，A 為最小）；兩步皆同則平手退注。不採用「視地區習慣」之模糊規則（決策見 D8） |
...
**特殊情況：三公 vs 三公**：若莊閒均持三公（Sam Gong，三張各別 mod 10 加總為0），則依 D8 同點比大小規則（第一步比最大單張花色：黑桃>紅心>方塊>梅花；花色亦同則比最大單張點數：K>Q>J>10>9>...>2>A）比較；若三步皆相同（完全相同花色牌，理論上不可能），則平手退注。（決策見 §15 D10）
...
| Post-Launch 3M | 2026-11-21（Launch+3M） | DAU ≥ 1,000、付費率 ≥ 1% | DAU < 300 或持續負成長 | Exec Sponsor |
...
| D10 | 2026-04-22 | 三公 vs 三公平手規則 | 若莊閒均持三公，依D8同點比大小規則（第一步比最大單張花色；花色相同再比最大單張點數K>Q>J>10>9>...>2>A）；三步皆同則平手退注（理論上不可能發生） | 三公對三公屬邊緣情境，必須明確定義避免Server邏輯未覆蓋導致遊戲卡死或爭議 | PM + Game Designer | §5.5 賠率表後 |
