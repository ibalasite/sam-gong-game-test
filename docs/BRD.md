...
| **文件版本** | v0.8-draft |
| **狀態** | DRAFT（BRD Fixer Round 7 完成）|
| **作者** | Evans Tseng（由 /devsop-idea 自動生成） |
| **日期** | 2026-04-21 |
| **建立方式** | /devsop-idea 自動生成；v0.8-draft 由 /devsop-autodev STEP-01 Review Round 7 自動修正 |
...
6. **結算與籌碼分配（三步驟）**：
   ...
   - Step 6c（分配）：依比牌結果，贏家閒家取回本金 + 從莊家收取 N× 下注額；莊家贏家則收取閒家下注額（扣抽水後）；結果廣播至所有 Client。
...
**混合結果結算順序**（莊家同局有贏有輸時）：Server 採原子性三步驟：(1) 先將所有閒家（含贏家與輸家）下注額全部收入底池；(2) 扣除 5% 抽水；(3) 依比牌結果依順時鐘順序逐一支付：贏家閒家從底池取回本金，莊家另行支付 N× 賠率；輸家閒家不取回；莊家最終淨增減 = 所有輸家下注總額 – 所有贏家賠率支出（含莊家破產規則 D13）。
...
| D13 | 2026-04-21 | 莊家結算破產處理順序 | 採順時鐘優先支付，餘額耗盡後按剩餘可用比例縮減 | 確保Server邏輯確定性，防止爭議 | PM + Engineering Lead | §5.5 結算邏輯 |
