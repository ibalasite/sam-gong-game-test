...
| **文件版本** | v0.5-draft |
| **狀態** | DRAFT（BRD Fixer Round 4 完成）|
| **作者** | Evans Tseng（由 /devsop-idea 自動生成） |
| **日期** | 2026-04-22 |
| **建立方式** | /devsop-idea 自動生成；v0.5-draft 由 /devsop-autodev STEP-01 Review Round 4 自動修正 |
...
| **同點比大小（F3 確定規則）** | 同點數時：**第一順位比最大單張花色**（黑桃 > 紅心 > 方塊 > 梅花）；花色相同時，再比最大單張點數（K>Q>J>10>9>...>2>A）；兩步皆同則平手退注。不採用「視地區習慣」之模糊規則（決策見 D8） |
